...
function redyuinit() {
    ...
    try {
        var time1 = 0;
        var myid = 0;
        var timeredyu1 = setInterval(function () {
            for (var i = 0; i < 10; i++) {
                container.appendChild(createALeaf(myid));
            }
            time1++;
            if (time1 > 30) {
                clearInterval(timeredyu1);
            }
        }, 2000);
 ...
 */
function createALeaf(myid) {
    /* Start by creating a wrapper div, and an empty img element */
    var leafDiv = document.createElement('div');
    var image = document.createElement('img');
    myid++;
    /* Randomly choose a leaf image and assign it to the newly created element */
    image.src = '/assets/t1/redyu/petal' + randomInteger(1, 10) + '.png';
    image.id = "redbag_" + myid; 
    leafDiv.setAttribute('class', 'redbag'); 
    /* Position the leaf at a random location along the screen */
    leafDiv.style.top = pixelValue(randomInteger(-200, -100));
    leafDiv.style.left = pixelValue(randomInteger(0, 620));
    ...
    //点击事件
    leafDiv.addEventListener("click", function () {
        show("ds");
        alert(1);
    }); 
    return leafDiv;
