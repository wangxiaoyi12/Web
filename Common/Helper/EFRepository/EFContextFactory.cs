...
namespace Common
{
    public sealed class EFContextFactory<T> where T : new()
    {
        #region Fields
        /// <summary>
        /// 定时间隔 ms
        ...
        #region 构造器--定时器  定时清理已结束的请求的上下文,暂时不用，web请求自动释放
        ...
        public static DbContext GetCurrentDbContext()
        {
            ...
            if (dbContext == null)  //线程在数据槽里面没有此上下文
            {
                ...
                dbContext = new T() as DbContext; //如果不存在上下文的话，创建一个EF上下文
                //我们在创建一个，放到数据槽中去
        ...
        public static DbContext GetHttpContextDbContext()
        {
            lock (lockobj)
            {
                ...
                {
                    divDataContext.Add(HttpContext.Current, new T() as DbContext);
                }
