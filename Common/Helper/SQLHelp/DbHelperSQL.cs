...
namespace Common
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        /// </summary>
        /// <param name="SQLString">sql语句</param>
        /// <param name="com">当前命令上下文</param>
        /// <param name="cmdParms">参数列表</param>
        /// <returns></returns>
        public static int ExecuteSql(string SQLString, SqlCommand com, params SqlParameter[] cmdParms)
        {
        ...
        public static object GetSingle(string SQLString, SqlCommand cmd)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (cmd = new SqlCommand(SQLString, connection))
                {
                    try
                    {
                        connection.Open();
        ...
        /// <param name="rowsAffected">影响的行数</param>
        /// <returns></returns>
        public static decimal RunProcedure_my(string storedProcName, IDataParameter[] parameters, out int rowsAffected)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                decimal result = 0;
                connection.Open();
                SqlCommand command = BuildIntCommand_my(connection, storedProcName, parameters);
                rowsAffected = (int)command.ExecuteNonQuery();
                result = Convert.ToDecimal(command.Parameters["ReturnValue"].Value);
                //Connection.Close();
                return result;
            }
        }
        ...
        /// <param name="parameters">存储过程参数</param>
        /// <returns>SqlCommand 对象实例</returns>
        private static SqlCommand BuildIntCommand_my(SqlConnection connection, string storedProcName, IDataParameter[] parameters)
        {
            SqlCommand command = BuildQueryCommand(connection, storedProcName, parameters);
            command.Parameters.Add(new SqlParameter("ReturnValue",
                SqlDbType.Money, 8, ParameterDirection.ReturnValue,
                false, 0, 0, string.Empty, DataRowVersion.Default, null));
            return command;
        }
        #endregion
