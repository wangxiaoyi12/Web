...
namespace Common
{
    ...
    public class XMLHelp
    {
        ...
        {
            FilePath = WebTools.MapPath(xmlFilePath);
            OpenXML();
        }
        #endregion
        ...
        #region 将xml转换成List<T> 集合，这里xml结构要符合，eg: <fields><field><code>id</code></field></fields>
        public List<T> GetList<T>(string root = "fields") where T : new()
        {
            ...
            foreach (XmlNode child in childs)
            {
                ...
                if (xe.HasChildNodes)
                {
                    ...
                    {
                        var p = ps.FirstOrDefault(a => a.Name == node.Name);

                        if (p != null && node.ChildNodes.Count > 0 && !string.IsNullOrEmpty(node.ChildNodes[0].Value))
                        {
                            p.SetValue(t, Convert.ChangeType(node.ChildNodes[0].Value, p.PropertyType), null);
                        }
                    }
                }
                list.Add(t);
        ...
        /// 设置节点值  
        /// </summary>  
        /// <param name="strNode">节点的名称 XPath表达式</param>  
        ...
        public void SetXmlNodeValue(string xmlNodePath, string xmlNodeValue)
        {
            try
            {
                ...
                {
                    //增加一个节点
                    AddChildNode("siteconfig", xmlNodePath.Replace("//", ""));
